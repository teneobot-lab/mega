  ...
  const onSubmit = async (data: ReceiptFormValues) => {
    ...
    try {
      await financeApi.createReceipt({
          ...
          sourceAccountId: data.items[0].accountId,
          amount: total,
          notes: data.memo || data.items[0].notes
      });
      toast.success("Penerimaan Kas Berhasil Disimpan!");
      setStatus("Posted");
      setTimeout(() => navigate("/cash-bank/receipt"), 1500);
    } catch (e: any) {
    ...
    <div className="flex flex-col h-screen bg-[#f3f4f6] text-[#333] font-sans overflow-hidden">
      ...
      <div className="bg-[#1e3a5f] text-white px-4 py-2 flex items-center justify-between shadow-md z-10">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button size="sm" variant="ghost" className="text-white hover:bg-white/10 h-8" onClick={() => navigate("/cash-bank/receipt")}>
            <X className="w-4 h-4" />
          </Button>
        </div>
      </div>
